...
return {
  {
    ...
    keys = {
      ...
      {
        "<LocalLeader>tf",
        function() require("custom.toggler").toggle(format_toggle, vim.api.nvim_get_current_buf()) end,
        desc = "Toggle format on current buffer",
      },
      {
        "<Leader>tf",
        function() require("custom.toggler").toggle(format_toggle) end,
    ...
    config = function(_, opts)
      ...
      require('custom.toggler').set(format_toggle, { buffer = "⌨", global = "⌨+", off = "" })

      local autoformat_group = vim.api.nvim_create_augroup("auto-format", { clear = true })
      vim.api.nvim_create_autocmd("BufWritePre", {
        group = autoformat_group,
        callback = function(event)
          if require("custom.toggler").enabled(format_toggle, event.buffer) then
            require("conform").format()
          end
        end,
      })
