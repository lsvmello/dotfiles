return {
  {
    ...
    keys = {
      ...
      { '<Leader>fc', function() require('telescope.builtin').find_files({ cwd = vim.fn.stdpath('config'), prompt_title = 'Find config files', hidden = true }) end, desc = 'Find config files' },
      { '<Leader>fC', function() require('telescope.builtin').live_grep({ cwd = vim.fn.stdpath('config'), prompt_title = 'Live grep in config files', hidden = true }) end, desc = 'Find in config files' },
      ...
      -- others
      { '<Leader>fp', function()
        require('telescope.builtin').find_files({ cwd = vim.fs.joinpath(vim.fn.stdpath('data'), 'lazy'), prompt = 'Find in plugins' })
      end, desc = 'Find in packages' },
      { '<Leader>fn', function()
        require('telescope.builtin').find_files({ cwd = vim.env.VIMRUNTIME, prompt = 'Find in Neovim' })
      end, desc = 'Find in Neovim runtime files' },
    },
  },
}
